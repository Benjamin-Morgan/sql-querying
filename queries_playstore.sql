...
-- 5. SELECT * FROM analytics WHERE reviews > 4.8 ORDER BY reviews DESC LIMIT 1;
-- 6. ELECT category, AVG(rating) FROM analytics GROUP BY category ORDER BY AVG DESC;
...
-- 9. SELECT * FROM analytics WHERE rating <= 3 AND reviews <= 10000;
-- 10.SELECT app_name FROM analytics WHERE price BETWEEN 0.1 AND 1 ORDER BY reviews DESC LIMIT 10;
...
-- 15. SELECT app_name, reviews, min_installs,  min_installs / reviews AS proportion FROM analytics nWHERE min_installs >= 100000 ORDER BY proportion DESC LIMIT 1;